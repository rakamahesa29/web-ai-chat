...
- **Real-time SSE streaming** for all providers with 8KB read buffers and token-by-token rendering.
- **Ollama keep-alive:** Models stay loaded in VRAM for 24 hours with TCP keep-alive connections, eliminating cold-start latency.
...
- **Dual search strategy:** Vector search (embeddings via Ollama `bge-m3:567m`) with automatic fallback to keyword search.
- **Hybrid search** combining vector similarity + keyword matching via RRF (Reciprocal Rank Fusion).
...
- **Optimized chunking:** 2000 chars per chunk (~512 tokens) with 512 char overlap for better retrieval quality.
- **Embedding cache:** Query embeddings are cached for 1 hour to avoid redundant Ollama API calls.
- **Memory-efficient vector search:** Pre-filters chunks by tag-matched brain IDs and processes in batches of 200 (prevents loading all embeddings into PHP memory).
- **FULLTEXT index** on `brains(title, content)` for fast keyword fallback.
...
- **Query-specific context:** Graph nodes matching the current query are injected into the prompt via multi-hop traversal.
- **Room Profile:** Top 20 most-frequent entities and their relationships are injected as a persistent `=== CONVERSATION PROFILE ===`, giving the AI long-term awareness of the conversation's themes, tech stack, and key decisions.
- Configurable max nodes, hops, and auto-cleanup.

### Memory Management (3-Tier Hierarchical System)
Three-tiered memory architecture designed to prevent context loss across long conversations:

| Tier | Source | Retention | Description |
|------|--------|-----------|-------------|
| **Tier 1: Room Synopsis** | `room_synopses` | Permanent | Meta-summary condensing all session summaries into themed categories (`[USER PROFILE]`, `[ACTIVE PROJECTS]`, `[TECHNICAL STACK]`, `[KEY DECISIONS]`, `[ONGOING TASKS]`). Auto-generated when 5+ session summaries accumulate. |
| **Tier 2: Session Summaries** | `room_summaries` | Permanent | Structured compression of old messages using category labels (`[DECISION]`, `[PREFERENCE]`, `[TECHNICAL]`, `[CODE]`, `[CONTEXT]`, `[TASK]`). Triggered when room exceeds 20 messages. |
| **Tier 3: Recent Messages** | `messages` | Active window | Raw conversation history — adaptive per model: 20 messages (Ollama local), 30 (Ollama Cloud), 40 (DeepSeek). |

- **Conversation Profile:** The Knowledge Graph's top 20 most-frequent entities and relationships are injected as a persistent `=== CONVERSATION PROFILE ===` block, giving the AI awareness of recurring topics, technologies, and decisions.
- **Adaptive summary injection:** 5 summaries for local models, 10 for cloud/DeepSeek.
- **Smart truncation:** Character-based limits adapted per provider (200K local, 500K cloud).
...
│   ├── RoomSummary.php            # Compressed memory summaries (Tier 2)
│   ├── RoomSynopsis.php           # Meta-summaries of summaries (Tier 1)
...
│   ├── CompressRoomMemory.php      # Memory compression job (Tier 2)
│   ├── GenerateRoomSynopsis.php   # Meta-synopsis generation (Tier 1)
