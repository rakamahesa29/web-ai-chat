...
@section('content')
    ...
    <div class="flex h-[calc(100dvh-0px)] bg-hermes-bg relative overflow-hidden" x-data="{ sidebarOpen: false }">
                ...
                @if (isset($rooms) && count($rooms) > 0)
                    @foreach ($rooms as $r)
                        <div class="group relative">
                            <a href="{{ route('chat.show', ['room' => $r->id]) }}" 
        ...
        <div class="flex-1 flex flex-col bg-hermes-bg overflow-hidden w-full h-[calc(100dvh-0px)]">
            ...
            <!-- Chat Header -->
            <header class="px-4 sm:px-6 py-3 border-b border-hermes-border bg-hermes-surface flex flex-col lg:flex-row lg:items-center justify-between gap-3 lg:gap-4">
                <div class="flex items-center gap-3 w-full lg:w-auto min-w-0">
                    ...
                    <div class="min-w-0 flex-1">
                        ...
                        <div class="flex flex-wrap items-center gap-2 text-[10px] sm:text-xs text-hermes-muted mt-1.5 sm:mt-2">
                ...
                <div class="flex items-center gap-2 sm:gap-3 w-full lg:w-auto shrink-0 overflow-x-auto pb-1 lg:pb-0 hide-scrollbar">
            ...
            @if(isset($isProcessing) && $isProcessing)
                <div id="processing-banner" class="bg-amber-500/10 border-b border-amber-500/20 px-4 py-2.5 flex items-center justify-between text-xs text-amber-400">
                    <div class="flex items-center gap-2">
                        <i data-lucide="loader-2" class="w-4 h-4 animate-spin shrink-0"></i>
                        <span>Harap tunggu, AI sedang memproses pesan Anda di background.</span>
                    </div>
                    <button type="button" onclick="stopBackgroundProcessing()" class="px-3 py-1 bg-amber-500/20 hover:bg-amber-500/30 text-amber-300 rounded-lg font-medium transition flex items-center gap-1.5 cursor-pointer">
                        <i data-lucide="square" class="w-3.5 h-3.5 fill-current"></i>
                        <span>Hentikan & Reset Lock</span>
                    </button>
                </div>
            @endif
                        ...
                        @if ($msg->sender_type == 'user')
                            <div class="flex flex-col items-end max-w-[90%] md:max-w-[80%]">
                                <div class="hermes-message-user">
                                    <div class="text-sm whitespace-pre-wrap break-words break-all leading-relaxed">{{ $msg->content }}</div>
                        ...
                        @else
                            <div class="flex flex-col items-start max-w-[90%] md:max-w-[75%] w-full">
                                ...
                                <div class="flex items-center gap-1 px-1 mt-1.5">
                                    ...
                                    <button onclick="exportGoogleDocs({{ $msg->id }})" class="hermes-btn-icon p-1 text-blue-400 hover:text-blue-300" title="Export to Google Docs">
                                        <i data-lucide="file-type-2" class="w-3.5 h-3.5"></i>
                                    </button>
                                    <button onclick="exportGoogleSheets({{ $msg->id }})" class="hermes-btn-icon p-1 text-emerald-400 hover:text-emerald-300" title="Export to Google Sheets">
            ...
            <div class="p-3 md:p-4 border-t border-hermes-border bg-hermes-surface">
                <form action="{{ route('chat.send', $room->id) }}" method="POST" id="chatForm">
                    @csrf
                    <input type="hidden" name="model_name" id="selectedModel" value="{{ request('model') ?? $room->model ?? 'deepseek' }}">
                    ...
                    <div class="relative flex items-end gap-2 bg-hermes-card border border-hermes-border rounded-xl p-2 focus-within:border-hermes-accent transition-colors">
                        ...
                        <!-- Stop Button (Swaps in place of Send button during processing) -->
                        <button type="button" id="stopBtn" onclick="stopCurrentGeneration()" class="hidden hermes-btn-icon bg-rose-600 text-white hover:bg-rose-700 transition-all shadow-md" title="Hentikan AI (Stop)">
    ...
    <script>
        ...
        function parseMarkdownSafe(text) {
            if (!text) return '';
            let processed = text.replace(/<i data-lucide="([^"]+)"><\/i>/g, '%%LUCIDE_ICON_$1%%');
            let html = typeof marked !== 'undefined' ? marked.parse(processed) : processed;
            return html.replace(/%%LUCIDE_ICON_([^%]+)%%/g, '<i data-lucide="$1"></i>');
        }
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.querySelectorAll('.message-wrapper').forEach(wrapper => {
                ...
                if (rawElem && parseElem) {
                    parseElem.innerHTML = parseMarkdownSafe(rawElem.value);
                }
            });
            lucide.createIcons();
        ...
        window.addEventListener('online', () => updateConnectionStatus('connected'));
        ...
        document.getElementById('chatForm').addEventListener('submit', async function(e) {
            ...
            sendBtn.classList.add('hidden', 'opacity-50', 'cursor-not-allowed');
            ...
            try {
                ...
                if (response.status === 429) {
                    ...
                    Swal.fire({
                        icon: 'warning',
                        title: 'Proses Background Berjalan',
                        text: errorData.message,
                        showCancelButton: true,
                        confirmButtonText: 'Hentikan & Reset Lock',
                        ...
                        if (event.startsWith('data: ')) {
                            ...
                            try {
                                ...
                                if (data.error) {
                                    ...
                                    botColDiv.innerHTML = `
                                        <div class="hermes-message-bot bg-red-500/10 border-red-500/30 mt-2">
                                            <div class="flex items-center gap-2 mb-3 font-semibold text-red-400">
                                                <i data-lucide="alert-triangle" class="w-5 h-5"></i>
                                                Connection or Service Error
                                            </div>
                                            <p class="text-sm text-hermes-muted mb-4">${data.error}</p>
                                        </div>
                                    `;
                                    lucide.createIcons();
                                    scrollToBottom();
                                ...
                                if (data.content !== undefined && (!data.type || data.type === 'chunk')) {
                                    ...
                                    if (!isKnowledgeGap) {
                                        ...
                                        textContainer.innerHTML = parseMarkdownSafe(displayText);
                                        scrollToBottom();
                                    }
                                }
                                ...
                                if (data.message_id && !isKnowledgeGap) {
                                    ...
                                    <div class="flex items-center gap-1 px-1 mt-1.5">
                                        ...
                                        <button onclick="exportGoogleDocs(${data.message_id})" class="hermes-btn-icon p-1 text-blue-400 hover:text-blue-300" title="Export to Google Docs">
                                            <i data-lucide="file-type-2" class="w-3.5 h-3.5"></i>
                                        </button>
                                        <button onclick="exportGoogleSheets(${data.message_id})" class="hermes-btn-icon p-1 text-emerald-400 hover:text-emerald-300" title="Export to Google Sheets">
                                            <i data-lucide="sheet" class="w-3.5 h-3.5"></i>
                                        </button>
        ...
        function finishStreaming() {
            ...
            sendBtn.classList.remove('hidden', 'opacity-50', 'cursor-not-allowed');
            const stopBtn = document.getElementById('stopBtn');
        ...
        async function stopBackgroundProcessing() {
            try {
                const response = await fetch("{{ route('chat.stopProcessing') }}", {
                    method: 'POST',
                    headers: {
        ...
        function appendUserMessage(content, hasCode = false, usedWebSearch = false) {
            ...
            <div class="flex flex-col items-end max-w-[90%] md:max-w-[80%]">
                <div class="hermes-message-user">
                    <div class="text-sm whitespace-pre-wrap break-words break-all leading-relaxed">${displayContent.replace(/</g, "&lt;").replace(/>/g, "&gt;")}</div>
                        ...
                        if (event.startsWith('data: ')) {
                            ...
                            try {
                                ...
                                if (data.content !== undefined && (!data.type || data.type === 'chunk')) {
                                    ...
                                    textContainer.innerHTML = parseMarkdownSafe(retryDisplayText);
                                    scrollToBottom();
                                }
        ...
        async function copyForDoc(btnElement) {
            const wrapper = btnElement.closest('.message-wrapper') || btnElement.closest('.flex.flex-col');
            const rawContent = wrapper ? wrapper.querySelector('.raw-message-content')?.value : null;
            const parsedContainer = wrapper ? wrapper.querySelector('.parsed-message-content') : null;
            ...
            if (!htmlContent && !plainText) {
                Swal.fire({
                    ...
                    position: 'top-end',
                    icon: 'error',
                    title: 'Tidak ada konten untuk disalin.',
                    showConfirmButton: false,
                    timer: 3000
                });
                return;
            ...
            // Clean up thesis eval tag if present
            htmlContent = htmlContent.replace(/\[THESIS_EVAL\][\s\S]*?\[\/THESIS_EVAL\]/g, '').trim();
            plainText = plainText.replace(/\[THESIS_EVAL\][\s\S]*?\[\/THESIS_EVAL\]/g, '').trim();

            const academicHtml = `
                <div style="font-family: 'Times New Roman', Times, serif; font-size: 12pt; line-height: 1.5; text-align: justify; color: #000000;">
                    ${htmlContent}
                </div>
            `.trim();

            try {
                const htmlBlob = new Blob([academicHtml], { type: 'text/html' });
                const textBlob = new Blob([plainText], { type: 'text/plain' });
                const clipboardItem = new ClipboardItem({
                    'text/html': htmlBlob,
                    'text/plain': textBlob
                ...
                Swal.fire({
                    ...
                    position: 'top-end',
                    icon: 'success',
                    title: 'Format Skripsi Siap Paste!',
                    text: 'Teks disalin dengan font Times New Roman 1.5 spasi.',
                    showConfirmButton: false,
                    timer: 3500,
        ...
        function exportDocx(messageId) {
            if (!messageId) {
                Swal.fire({
                    toast: true,
                    position: 'top-end',
                    icon: 'error',
                    title: 'ID pesan tidak ditemukan.',
                    showConfirmButton: false,
                    timer: 3000
                });
                return;
            ...
            window.location.href = `/chat/message/${messageId}/export-docx`;
            
            Swal.fire({
                toast: true,
                position: 'top-end',
                icon: 'info',
                title: 'Mengunduh file .docx...',
                showConfirmButton: false,
                timer: 3000
            });
        }

        async function exportGoogleDocs(messageId) {
            if (!messageId) return;

            Swal.fire({
                title: 'Mengekspor ke Google Docs...',
                text: 'Sedang membuat dokumen baru di akun Google Anda.',
                allowOutsideClick: false,
                didOpen: () => {
                    Swal.showLoading();
                }
            ...
            try {
                const res = await fetch(`/chat/message/${messageId}/export-google-docs`, {
                    method: 'POST',
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
                ...
                if (res.status === 401 && data.auth_url) {
                    Swal.fire({
                        ...
                        text: 'Anda perlu menghubungkan akun Google Anda terlebih dahulu untuk mengekspor dokumen ini.',
                        icon: 'info',
                        showCancelButton: true,
                        confirmButtonText: 'Hubungkan Akun Google',
                ...
                if (data.status === 'success' && data.url) {
                    Swal.fire({
                        title: 'Google Doc Berhasil Dibuat!',
                        text: 'Dokumen baru telah berhasil dibuat di Google Drive Anda.',
                        icon: 'success',
                        showCancelButton: true,
                        confirmButtonText: 'Buka Google Doc',
                        cancelButtonText: 'Tutup',
                        customClass: {
                            popup: 'my-swal-popup',
                            title: 'my-swal-title'
                    ...
                    Swal.fire({
                        ...
                        text: data.message || 'Terjadi kesalahan saat menghubungi Google Docs API.',
                        icon: 'error'
                    });
                }
            } catch (err) {
        ...
        async function exportGoogleSheets(messageId) {
            if (!messageId) return;

            Swal.fire({
                title: 'Mengekspor ke Google Sheets...',
                text: 'Sedang membuat spreadsheet baru di akun Google Anda.',
                allowOutsideClick: false,
                didOpen: () => {
                    Swal.showLoading();
                }
            ...
            try {
                const res = await fetch(`/chat/message/${messageId}/export-google-sheets`, {
                    method: 'POST',
                    headers: {
                        "Content-Type": "application/json",
                        "X-CSRF-TOKEN": "{{ csrf_token() }}"
                ...
                if (res.status === 401 && data.auth_url) {
                    Swal.fire({
                        title: 'Hubungkan Akun Google',
                        text: 'Anda perlu menghubungkan akun Google Anda terlebih dahulu untuk mengekspor spreadsheet ini.',
                        icon: 'info',
                        showCancelButton: true,
                        confirmButtonText: 'Hubungkan Akun Google',
                        cancelButtonText: 'Batal',
                ...
                if (data.status === 'success' && data.url) {
                    Swal.fire({
                        title: 'Google Sheet Berhasil Dibuat!',
                        text: 'Spreadsheet baru telah berhasil dibuat di Google Drive Anda.',
                        icon: 'success',
                        showCancelButton: true,
                    ...
                    Swal.fire({
                        title: 'Gagal Mengekspor',
                        text: data.message || 'Terjadi kesalahan saat menghubungi Google Sheets API.',
                        icon: 'error'
                    });
                }
            } catch (err) {
