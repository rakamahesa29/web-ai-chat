...
class GraphManager
{
    ...
    /**
     * Generate a compact "room personality profile" from the knowledge graph.
     * Returns the top entities and their relationships as persistent context.
     */
    public function getRoomProfile(int $roomId): array
    {
        $minNodes = 5;
        $nodeCount = KnowledgeNode::where('room_id', $roomId)->count();

        if ($nodeCount < $minNodes) {
            return ['success' => false, 'reason' => 'Insufficient graph data for profile'];
        }
        ...
        $nodeIds = $topNodes->pluck('id')->toArray();
        ...
        $byType = $topNodes->groupBy('node_type');
        $profile = "=== CONVERSATION PROFILE ===\n";
        $profile .= "(Persistent context from conversation history)\n\n";

        foreach ($byType as $type => $nodes) {
            $label = ucfirst($type) . 's';
            $items = $nodes->map(fn($n) => $n->content . ' (' . $n->frequency . 'x)')->implode(', ');
            $profile .= "**{$label}**: {$items}\n";
        }
        ...
        if ($edges->isNotEmpty()) {
            ...
            foreach ($edges as $edge) {
                $source = $topNodes->firstWhere('id', $edge->source_node_id);
                $target = $topNodes->firstWhere('id', $edge->target_node_id);
                if ($source && $target) {
                    $profile .= "- {$source->content} → {$edge->relation} → {$target->content}\n";
                }
            }
        }
        ...
        return [
            ...
            'token_estimate' => intval(strlen($profile) / 4),
        ];
    }
